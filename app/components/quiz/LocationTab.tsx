...
export default LocationTab;// components/quiz/LocationTab.tsx
'use client';
...
const LocationTab = ({ onAnswer, answer }: LocationTabProps) => {
  ...
  useEffect(() => {
    const initAutocomplete = () => {
      if (window.google && window.google.maps && window.google.maps.places) {
        ...
        // Store services in window for access
        (window as any).autocompleteService = autocompleteService;
  ...
  const searchPlaces = async (query: string) => {
    ...
    try {
      // For demo purposes, using a mock autocomplete service
      // In production, replace with actual Google Places API
      const mockSuggestions = [
        { place_id: '1', description: `${query}, India` },
        { place_id: '2', description: `${query}, United States` },
        { place_id: '3', description: `${query}, United Kingdom` },
        { place_id: '4', description: `${query}, Australia` },
        { place_id: '5', description: `${query}, Canada` },
      ].filter(place => 
  ...
  const handleSuggestionClick = (suggestion: any) => {
    ...
    onAnswer(suggestion.description);
    setShowSuggestions(false);
    setSuggestions([]);
  };
  ...
  // Update answer when location changes
  useEffect(() => {
    if (location && !showSuggestions) {
      onAnswer(location);
    }
