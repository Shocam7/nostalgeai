...
const PAGES_TO_FETCH = 5; // first 5 pages

// Jaro-Winkler functions (same as trending)
...
export async function GET(req: Request) {
  try {
    ...
    if (!query) {
      // if no query, return empty results (or you may want trending)
      return NextResponse.json({ results: [] });
    }

    const fetches = [];
    for (let p = 1; p <= PAGES_TO_FETCH; p++) {
      const url = `${TMDB_BASE}/search/movie?query=${encodeURIComponent(query)}&page=${p}`;
    ...
    const jsons = await Promise.all(responses.map((r) => {
      if (!r.ok) throw new Error("TMDB search fetch failed");
      return r.json();
    }));
    ...
    // compute relevance (F1) using title-only jaro-winkler [0..1]
    const scored = unique.map((mv) => {
      const title = (mv.title || "").toString();
      const relevance = jaroWinkler(title, query);
      return { ...mv, __relevance: relevance };
    });

    // normalize popularity
    const maxPop = scored.reduce((acc, mv) => Math.max(acc, mv.popularity ?? 0), 0) || 1;
    const items = scored.map((mv) => {
      const popularity_norm = (mv.popularity ?? 0) / maxPop;
      const relevance = mv.__relevance ?? 0;
      const score = popularity_norm * 0.3 + relevance * 0.7; // ScoreFormula: C
